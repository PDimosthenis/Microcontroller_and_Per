...
// Δημιουργία πίνακα 16 θέσεων για να υποστηρίζονται πολλαπλά interrupts ταυτόχρονα
static void (*GPIO_callbacks[16])(int status) = {0};
...
void gpio_set_mode(Pin pin, PinMode mode) {
    ...
    RCC->AHB1ENR |= 1UL<<GET_PORT_INDEX(pin); // enable clock output
    RCC->APB2ENR |= RCC_APB2ENR_SYSCFGEN;     // Enable clock for interrupts
    DBGMCU->CR |= DBGMCU_CR_DBG_SLEEP | DBGMCU_CR_DBG_STOP | DBGMCU_CR_DBG_STANDBY;
    ...
    switch(mode) {
        ...
        case PullDown:
            MODIFY_REG(p->MODER, 3UL<<((pin_index)*2), 0UL<<((pin_index)*2));  
            MODIFY_REG(p->PUPDR, 3UL<<((pin_index)*2), 2UL<<((pin_index)*2)); // Διόρθωση: Το PullDown είναι 0b10 (2), όχι 10.
            break;
    }
}
...
void gpio_set_callback(Pin pin, void (*callback)(int status)) {
    ...
    uint32_t IRQ_port_num = GET_PORT_INDEX(pin);
    uint32_t IRQ_pin_index = GET_PIN_INDEX(pin);
    uint32_t EXTI_port_set = IRQ_port_num << ((IRQ_pin_index % 4) * 4);
    
    // Αποθήκευση της callback στη σωστή θέση του πίνακα
    GPIO_callbacks[IRQ_pin_index] = callback;
    
    uint32_t prioritygroup = NVIC_GetPriorityGrouping();
    uint32_t priority = NVIC_EncodePriority(prioritygroup, 1, 1);
    
    if(IRQ_pin_index <= 3) {
        // Clear previous routing bits for this line and set new ones
        SYSCFG->EXTICR[0] &= ~(0xF << ((IRQ_pin_index % 4) * 4)); 
        SYSCFG->EXTICR[0] |= EXTI_port_set;
    } else if(IRQ_pin_index <= 7) {
        SYSCFG->EXTICR[1] &= ~(0xF << ((IRQ_pin_index % 4) * 4));
        SYSCFG->EXTICR[1] |= EXTI_port_set;
    } else if(IRQ_pin_index <= 11) {
        SYSCFG->EXTICR[2] &= ~(0xF << ((IRQ_pin_index % 4) * 4));
        SYSCFG->EXTICR[2] |= EXTI_port_set;
    } else {
        SYSCFG->EXTICR[3] &= ~(0xF << ((IRQ_pin_index % 4) * 4));
        SYSCFG->EXTICR[3] |= EXTI_port_set;
    }

    if(IRQ_pin_index == 0)      { NVIC_SetPriority(EXTI0_IRQn, priority); NVIC_EnableIRQ(EXTI0_IRQn); }
    else if(IRQ_pin_index == 1) { NVIC_SetPriority(EXTI1_IRQn, priority); NVIC_EnableIRQ(EXTI1_IRQn); }
    else if(IRQ_pin_index == 2) { NVIC_SetPriority(EXTI2_IRQn, priority); NVIC_EnableIRQ(EXTI2_IRQn); }
    else if(IRQ_pin_index == 3) { NVIC_SetPriority(EXTI3_IRQn, priority); NVIC_EnableIRQ(EXTI3_IRQn); }
    else if(IRQ_pin_index == 4) { NVIC_SetPriority(EXTI4_IRQn, priority); NVIC_EnableIRQ(EXTI4_IRQn); }
    else if(IRQ_pin_index <= 9) { NVIC_SetPriority(EXTI9_5_IRQn, 3); NVIC_EnableIRQ(EXTI9_5_IRQn); }
    else { NVIC_SetPriority(EXTI15_10_IRQn, 3); NVIC_EnableIRQ(EXTI15_10_IRQn); }
}

// ============================================================================
// Ανεξάρτητοι IRQ Handlers που ελέγχουν τις αντίστοιχες γραμμές
// ============================================================================
...
void EXTI1_IRQHandler(void){
    if(EXTI->PR & (1<<1)) {
        EXTI->PR |= (1<<1); 
        if(GPIO_callbacks[1]) GPIO_callbacks[1](1);
    }
}
...
void EXTI2_IRQHandler(void){
    if(EXTI->PR & (1<<2)) {
        EXTI->PR |= (1<<2); 
        if(GPIO_callbacks[2]) GPIO_callbacks[2](2);
    }
}
...
void EXTI3_IRQHandler(void){
    if(EXTI->PR & (1<<3)) {
        EXTI->PR |= (1<<3); 
        if(GPIO_callbacks[3]) GPIO_callbacks[3](3);
    }
}
...
void EXTI4_IRQHandler(void){
    if(EXTI->PR & (1<<4)) {
        EXTI->PR |= (1<<4); 
        if(GPIO_callbacks[4]) GPIO_callbacks[4](4);
    }
}

void EXTI9_5_IRQHandler(void){
    // Ελέγχει ποιο pin (από το 5 έως το 9) πυροδότησε το interrupt
    for(int i = 5; i <= 9; i++) {
        if(EXTI->PR & (1<<i)) {
            EXTI->PR |= (1<<i); // Καθαρισμός του flag
            if(GPIO_callbacks[i]) GPIO_callbacks[i](i); // Εκτέλεση της αντίστοιχης συνάρτησης
        }
    }
}

void EXTI15_10_IRQHandler(void){
    // Ελέγχει ποιο pin (από το 10 έως το 15) πυροδότησε το interrupt
    for(int i = 10; i <= 15; i++) {
        if(EXTI->PR & (1<<i)) {
            EXTI->PR |= (1<<i); 
